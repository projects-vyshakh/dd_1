...
@endif
<input type="hidden" id="country-hidden" value="India">
							...
							<div class="form-group dd_fromgroup_MG_0 ">
								...
								<div class="col-sm-4">
							 	...
							 	<span class="symbol required"></span> 	
									...
									<div class="col-sm-8">
										<span class="">
											{!! Form::select('state', null, (!empty($patientData))?$patientState: Input::old('state'), $attributes = array('class' => 'form-control','id'=>'state')); !!}
										</span>
									</div>
								</div>
							</div>
...
@section('scripts')
	...
	<script>
		$(document).ready(function() {
			...
			var patientData = <?php echo json_encode($patientData); ?>;

			if(patientData=='' || patientData==null){
				/*Dynamically adding state responding to country and also keeping selected patientData of state*/
				
				var countryHidden = $('#country-hidden').val();
				//alert(countryHidden);
				console.log($('#country').val(countryHidden).attr("selected", "selected"));
			}
			else{
